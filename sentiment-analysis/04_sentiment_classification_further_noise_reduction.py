...
class SentimentNetwork:
    def __init__(self, reviews, labels, hidden_nodes=10, learning_rate=0.1,
                 min_count=20, polarity_cutoff=0.05):
        ...
        self.analisys_data(reviews, labels)
    ...
    def pre_process_data(self, reviews, labels, min_count, polarity_cutoff):
        ...
        review_vocab = {word for (word, cnt), (word_, ratio)
                        in zip(self.total_counts.items(), self.pos_neg_ratios.items())
                        if word == word_ and cnt > min_count and math.fabs(ratio) > polarity_cutoff}
    ...
    def train(self, training_reviews, training_labels):
        ...
        for i in range(len(training_reviews)):
            ...
            for nonzero_index in np.nonzero(self.layer_0)[1]:
                layer_1 += self.weights_0_1[nonzero_index]
            ...
            for nonzero_index in np.nonzero(self.layer_0)[1]:
                self.weights_0_1[nonzero_index] -= layer_1_delta[0] * self.learning_rate  # update input-to-hidden weights with gradient descent step
    ...
    def analisys_data(self, reviews, labels):
        ...
        for review, label in zip(reviews, labels):
            ...
            self.total_counts.update(review.split())

        # Calculate the ratios of positive and negative uses of the most common words
        for word, cnt in list(self.total_counts.items()):
